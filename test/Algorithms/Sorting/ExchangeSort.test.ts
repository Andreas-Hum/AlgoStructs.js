import { exchangeSort } from '../../../src/Algorithms/Sorting';
    ...
    test('should sort numbers in ascending order', () => {
        fc.assert(
            fc.property(fc.array(fc.integer()), (array) => {
                const sortedArray: number[] = exchangeSort({ array, compare: (a, b) => a - b });
    ...
    test('should sort strings in alphabetical order', () => {
        fc.assert(
            fc.property(fc.array(fc.string()), (array) => {
                const sortedArray: string[] = exchangeSort({ array, compare: (a, b) => a.localeCompare(b) });
                for (let i: number = 1; i < sortedArray.length; i++) {
    ...
    test('should handle array with one element', () => {
        fc.assert(
            fc.property(fc.integer(), (value) => {
                const sortedArray: number[] = exchangeSort({ array: [value], compare: (a, b) => a - b });
